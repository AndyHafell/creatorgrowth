...
export class PlaybackManager {
	...
	/** Review-speed playback. Rebases the clock so a mid-play change is seamless. */
	...
	/** d-key cycle: 1x → 2x → 3x → 1x. Returns the new speed. */
	cycleSpeed(): number {
		const index = SPEED_CYCLE.indexOf(this.speed);
		const next = SPEED_CYCLE[(index + 1) % SPEED_CYCLE.length];
		this.setSpeed({ speed: next });
		return next;
	}
	...
	private updateTime = (): void => {
		...
		const elapsedSeconds =
			((performance.now() - this.playbackStartWallTime) / 1000) * this.speed;
		const rawTime =
