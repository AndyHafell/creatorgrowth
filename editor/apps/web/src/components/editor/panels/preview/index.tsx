...
import { MagicRegionOverlay } from "./magic-region-overlay";
import { MagicReviewHud } from "@/components/editor/magic-review/magic-review-hud";
		...
		if (canvasRef.current && renderTree && !renderingRef.current) {
			...
			if (
				...
				renderer
					...
					.catch((error) => {
						// without this, one bad frame freezes the preview forever
						console.error("[editor] preview render failed:", error);
						renderingRef.current = false;
