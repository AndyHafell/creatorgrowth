...
import { enforceMainTrackStart, resolveTrackPlacement } from "@/lib/timeline/placement";
...
export function computeDropTarget({
	...
	// Internal move hovering a track that can't host this element type (e.g. an
	// effect clip dragged onto the main video track): keep the move horizontal
	// on the source track instead of resolving to a brand-new track — that was
	// the "effect clips won't move over the Raw Cut track" jank. Deliberate
	// new-track gestures (dragging above/below all tracks) hit the
	// !trackAtMouse branch before this and still work.
	const requiredTrackType = getTrackTypeForElementType({ elementType });
	if (sourceTrackId && track.type !== requiredTrackType) {
		const sourceTrackIndex = orderedTracks.findIndex(
			({ id }) => id === sourceTrackId,
		);
		const sourceTrack =
			sourceTrackIndex >= 0 ? orderedTracks[sourceTrackIndex] : null;
		if (
			sourceTrack &&
			sourceTrack.type === requiredTrackType &&
			...
			return {
				...
				isNewTrack: false,
				insertPosition: null,
				xPosition: enforceMainTrackStart({
					tracks,
	...
	const trackHeight = getTrackHeight({ type: track.type, trackId: track.id });
	const placementResult = resolveTrackPlacement({
		tracks,
		elementType,
		timeSpans: [{ startTime: xPosition, duration: elementDuration, excludeElementId }],
...
export function getDropLineY({
	...
	for (let i = 0; i < safeTrackIndex; i++) {
		y +=
			getTrackHeight({ type: tracks[i].type, trackId: tracks[i].id }) +
