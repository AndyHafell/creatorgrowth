...
export function TimelineElement({
	...
	const baseTrackHeight = getTrackHeight({ type: track.type, trackId: track.id });
...
function AudioElementContent({
	...
	// Sidestep the flex chain that collapses size-full to 0 (parent's
	// items-center doesn't stretch children) — match video clips by reading
	// the track height directly and setting it inline. This is what makes the
	// canvas inside <AudioWaveform> a non-zero box to draw into.
		...
		return (
			...
			>
				<AudioWaveform
					...
					visibleDurationSec={element.duration / TICKS_PER_SECOND}
	...
	const missingReason =
		element.sourceType === "upload" && !mediaAsset
			? "media asset missing — re-Add to Timeline"
			: "audio not loaded";
	return (
		<div
			className="relative w-full overflow-hidden"
			style={{ height: `${trackHeight}px` }}
		>
			<MediaElementHeader name={element.name} hasFade={false} />
			<span className="absolute inset-x-2 bottom-1 truncate text-[10px] text-yellow-200/90">
				⚠ {missingReason}
			</span>
		</div>
	);
