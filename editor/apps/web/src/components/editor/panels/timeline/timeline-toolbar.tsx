...
function ToolbarRightSection({
	...
	const rollEditingEnabled = useTimelineStore((s) => s.rollEditingEnabled);
	const toggleSnapping = useTimelineStore((s) => s.toggleSnapping);
	const toggleRippleEditing = useTimelineStore((s) => s.toggleRippleEditing);
	const toggleRollEditing = useTimelineStore((s) => s.toggleRollEditing);
		...
		<div className="flex items-center gap-1">
			...
			<TooltipProvider delayDuration={500}>
				...
				<ToolbarButton
					...
					isActive={rollEditingEnabled}
					tooltip="Roll edit — drag a cut, both clips move"
					onClick={() => toggleRollEditing()}
				/>
			</TooltipProvider>
			...
			<div className="flex items-center gap-1">
				<TooltipProvider delayDuration={500}>
					<Tooltip>
						...
						<TooltipContent>Zoom out (Z)</TooltipContent>
					</Tooltip>
				</TooltipProvider>
				<Slider
					className="w-28"
				...
				<TooltipProvider delayDuration={500}>
					<Tooltip>
						...
						<TooltipContent>Zoom in (X)</TooltipContent>
					</Tooltip>
				</TooltipProvider>
			</div>
...
/** Review-speed buttons — same state the d key cycles through. */
function PlaybackSpeedControl() {
	const editor = useEditor();
	const speed = useEditor((e) => e.playback.getSpeed());
	...
	return (
		<TooltipProvider delayDuration={500}>
			<Tooltip>
				<TooltipTrigger asChild>
					<div className="flex items-center gap-0.5">
						{[1, 2, 3].map((s) => (
							<Button
								key={s}
								variant={speed === s ? "secondary" : "text"}
								size="sm"
								className="h-7 rounded-sm px-1.5 text-[11px] font-semibold"
								onClick={() => editor.playback.setSpeed({ speed: s })}
							>
								{s}×
							</Button>
						))}
				...
				<TooltipContent>Playback speed (D cycles)</TooltipContent>
			</Tooltip>
		</TooltipProvider>
	);
}
...
/**
 * Vertical zoom for track row heights. Independent from the horizontal zoom
 * (which scales the time axis). Persists via timeline-store.
 */
function TrackHeightControl() {
	const scale = useTimelineStore((s) => s.trackHeightScale);
	const setScale = useTimelineStore((s) => s.setTrackHeightScale);
	const min = 0.3;
	const max = 2.5;
	return (
		<TooltipProvider delayDuration={500}>
			<Tooltip>
				...
				<TooltipContent>Track height ({scale.toFixed(2)}×)</TooltipContent>
			</Tooltip>
		</TooltipProvider>
	);
}
