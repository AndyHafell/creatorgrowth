...
export function useEditorActions() {
	...
	useActionHandler(
		...
		() => {
			const speed = editor.playback.cycleSpeed();
			toast(`${speed}×`, {
				id: "playback-speed",
				duration: 1200,
			});
	...
	// Timeline zoom lives as local state in useTimelineZoom (hook called from
	// Timeline.tsx), so we route via a window event the Timeline subscribes to.
	useActionHandler(
		"timeline-zoom-in",
		() => {
			window.dispatchEvent(
				new CustomEvent("timeline-zoom-action", {
					detail: { direction: "in" },
				}),
			);
		},
