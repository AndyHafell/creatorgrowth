...
/**
 * Pitch-preserved tempo stretch of a ready clip buffer — review-speed
 * playback (2x/3x) runs every clip's prepared buffer through one extra pass:
 * output is buffer.duration/tempo seconds at the same pitch.
 */
export async function stretchBufferTempo({
	buffer,
	tempo,
}: {
	buffer: AudioBuffer;
	...
	if (Math.abs(tempo - 1) < RATE_EPSILON) {
		return buffer;
	}
	const numChannels = Math.max(1, Math.min(2, buffer.numberOfChannels));
	const outputSamples = Math.max(1, Math.ceil(buffer.length / tempo));
