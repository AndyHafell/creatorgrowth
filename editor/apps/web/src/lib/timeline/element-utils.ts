...
import { buildDefaultEffectInstance, effectsRegistry } from "@/lib/effects";
...
export function buildEffectElement({
	...
	const definition = effectsRegistry.has(effectType)
		? effectsRegistry.get(effectType)
		: null;
	return {
		type: "effect",
...
function buildUploadAudioElement({
	...
	const element: CreateUploadAudioElement = {
		...
		trimEnd: 0,
		sourceDuration: duration,
		// Upstream DEFAULTS.element.volume is 0 (intended for video clips that
		// shouldn't compete with voiceover). Audio elements explicitly added by
		// the user should be audible by default.
		volume: 1,
		muted: false,
	};
	if (buffer) {
...
export function buildLibraryAudioElement({
	...
	const element: CreateLibraryAudioElement = {
		...
		trimEnd: 0,
		sourceDuration: duration,
		// See buildUploadAudioElement — upstream default is 0, override to 1.
		volume: 1,
		muted: false,
	};
	if (buffer) {
