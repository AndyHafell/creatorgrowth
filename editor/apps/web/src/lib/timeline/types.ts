...
export interface ComputeDropTargetParams {
	...
	/**
	 * For internal moves: the track the element is being dragged from. When the
	 * hovered track can't host the element type (e.g. an effect clip over the
	 * main video track), the drop falls back to a horizontal retime on this
	 * track instead of spawning a new one.
	 */
